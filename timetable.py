...
class Cohort:
    ...
    def schedule_in_room(self, rooms, day, start, end):
        ...
        if not room_scheduled:
            print(f'{self} could not be scheduled in any room during this time.')
...
cohorts = [Cohort('C1', 90), Cohort('C2', 60), Cohort('C3', 40), Cohort('C4', 92)]
...
timetable.schedule_cohort(cohorts[1], 'Monday', 9, 11)
